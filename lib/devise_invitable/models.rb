    ...
    module Models
        ...
        module Invitable
            ...
            included do
                ...
                # puts "models::invitation_not_accepted: bool=[#{(defined?(Dynamoid) && defined?(Dynamoid::Document)).inspect}], Dynamoid.defined?=[#{defined?(Dynamoid)}], Document.defined?=[#{defined?(Dynamoid::Document)}]"

                if (defined?(Mongoid) && defined?(Mongoid::Document)) or (defined?(ActiveRecord) && defined?(ActiveRecord::Base))
                    scope :no_active_invitation, lambda { where(invitation_token: nil) }
                elsif defined?(Dynamoid) && defined?(Dynamoid::Document)
                ...
                if defined?(Mongoid) && defined?(Mongoid::Document) && self < Mongoid::Document
                ...
                elsif defined?(ActiveRecord) && defined?(ActiveRecord::Base)
                    ...
                    def self.created_by_invite
                        lambda { where(:invitation_created_at.ne => nil) }
                    end

                    def self.invitation_not_accepted
                        where('invitation_accepted_at.null': true, 'invitation_token.not_null': true).count
                    end
            ...
            def invite!(invited_by = nil, options = {})
                # TODO: REMOVE THIS DEBUGGING STATEMENT
                # puts "invite!: invited_by=[#{invited_by.inspect}], options=[#{options.inspect}]"
            ...
            def invitation_due_at
                ...
                time.to_datetime + self.class.invite_for
            end
            ...
            def invitation_period_valid?
                ...
                self.class.invite_for.to_i.zero? || (time && time.to_datetime.utc >= self.class.invite_for.ago)
            end
            ...
            def generate_invitation_token
                # TODO: REMOVE THIS DEBUGGING STATEMENT
                # puts "generate_invitation_token: inside"

                raw, enc = Devise.token_generator.generate(self.class, :invitation_token)

                # TODO: REMOVE THIS DEBUGGING STATEMENT
                # puts "generate_invitation_token: raw=[#{raw.inspect}] enc=[#{enc.inspect}]"
            ...
            module ClassMethods
                ...
                def _invite(attributes = {}, invited_by = nil, options = {}, &block)
                    ...
                    # TODO: REMOVE THIS DEBUGGING STATEMENT
                    # puts "_invite: attributes=[#{attributes.inspect}], invited_by=[#{invited_by.inspect}], options=[#{options.inspect}], block=[#{block.inspect}]"
                    ...
                    invitable.class.attributes.each_with_object({}) do |(attribute, options), res|
                        if attributes.key?(attribute)
                            ...
                            #     invitable[attribute] = options[:default]
                        end
                    end

                    invitable.invited_by = invited_by unless invited_by.nil?
                ...
                def invite!(attributes = {}, invited_by = nil, options = {}, &block)
                    ...
                    # TODO: REMOVE THIS DEBUGGING STATEMENT
                    # puts "invite!: attributes=[#{attributes.inspect}], invited_by=[#{invited_by.inspect}], options=[#{options.inspect}], block=[#{block.inspect}]"

                    attr_hash = ActiveSupport::HashWithIndifferentAccess.new(attributes.to_h)

                    # TODO: REMOVE THIS DEBUGGING STATEMENT
                    # puts "invite!: attr_hash=[#{attr_hash.inspect}]"
